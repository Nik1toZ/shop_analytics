# data_ingestion/upload_products.py
import os
import io
import pandas as pd
...
# 1) Инициализация MinIO-клиента (аналог get_s3_client из utils/s3_client.py)
...
def upload_products_from_parquet(local_parquet_path: str):
    ...
    # читаем локальный Parquet (если нужно, можно конвертировать CSV → Parquet)
    df = pd.read_parquet(local_parquet_path)  # или pd.read_csv(), а потом df.to_parquet
    ...
    # формируем key
    # можно разбить по категориям или просто положить в корень products/
...
if __name__ == "__main__":
    # перед запуском задайте путь, например: python upload_products.py ./data/products.parquet
    import sys
    if len(sys.argv) < 2:
